...
@section('content')
    <div class="container-fluid ringkasan-siska-page">
        ...
        <div class="card shadow-sm mb-4 siska-filter-card" id="activeFilters"
            ...
            data-filter-wilker="{{ $currentFilters['wilker'] }}" data-filter-status="{{ $currentFilters['status'] }}"
                            ...
                            <div class="d-flex flex-wrap gap-2 mt-3">
                                ...
                                @if ($currentFilters['wilker'])
                                    <span class="siska-chip siska-chip-indigo">
                                        ...
                                        <a href="{{ route('reports.ringkasan-siska.index', array_merge($currentFilters, ['filter_wilker' => ''])) }}"
                                            class="ms-1 text-inherit">×</a>
                                    </span>
                                @endif
                                        ...
                                        @forelse($depWilayahMatrix as $row)
                                            ...
                                            <tr>
                                                <td class="fw-semibold siska-matrix-dep-col">
                                                    <span class="drill-trigger" data-type="departemen"
                                                ...
                                                @foreach ($allWilayah as $wNama)
                                                    ...
                                                    <td class="text-center siska-matrix-cell {{ $cnt > 0 ? 'has-data' : 'no-data' }}"
                                                        @if ($cnt > 0 && $maxVal > 0) style="--heat: {{ round(($cnt / $maxVal) * 100) }}%" @endif>
                                                        @if ($cnt > 0)
                                ...
                                <tbody>
                                    @forelse($perPT as $item)
                                        <tr class="drill-trigger siska-row-clickable" data-type="pt"
                                            ...
                                            <td><span class="fw-semibold">{{ $item['singkatan'] }}</span><br><small
                                                    class="text-muted">{{ $item['label'] }}</small></td>
                                            <td><small>{{ $item['bidang'] }}</small></td>
                                            <td class="text-end"><span
                                                    class="siska-num-badge bg-primary">{{ number_format($item['jumlah']) }}</span>
                                            </td>
                                            <td class="text-end"><span
                                                    class="badge bg-primary bg-opacity-75">{{ $totalKaryawan > 0 ? round(($item['jumlah'] / $totalKaryawan) * 100, 1) : 0 }}%</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                ...
                                <tbody>
                                    @forelse($perBidangUsaha as $item)
                                        <tr class="drill-trigger siska-row-clickable" data-type="bidang"
                                            ...
                                            <td class="text-end"><span
                                                    class="badge bg-success bg-opacity-75">{{ $totalKaryawan > 0 ? round(($item['jumlah'] / $totalKaryawan) * 100, 1) : 0 }}%</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
            ...
            <div class="col-lg-6">
                <div class="card shadow-sm h-100 siska-section-card">
                    <div class="card-header siska-section-header"
                        ...
                        <i class="fas fa-map-marker-alt me-2" style="opacity:.85;"></i>Pusat & Cabang (Wilayah Kerja)
                    </div>
                    <div class="card-body">
                                ...
                                <tbody>
                                    @forelse($perPusatCabang as $item)
                                        <tr class="drill-trigger siska-row-clickable" data-type="wilker"
                                            ...
                                            <td class="text-end"><span class="siska-num-badge"
                                                    style="background:#4c6ef5">{{ number_format($item['jumlah']) }}</span>
                                            </td>
                                            <td class="text-end"><span class="badge"
                                                    style="background:#4c6ef5">{{ $totalKaryawan > 0 ? round(($item['jumlah'] / $totalKaryawan) * 100, 1) : 0 }}%</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
            ...
            <div class="col-lg-6">
                <div class="card shadow-sm h-100 siska-section-card">
                    <div class="card-header siska-section-header"
                        ...
                        <i class="fas fa-sitemap me-2" style="opacity:.85;"></i>Unit Kerja (Area)
                    </div>
                    <div class="card-body">
                                ...
                                <tbody>
                                    @forelse($perDepartemen as $item)
                                        <tr class="siska-row-clickable">
                                            ...
                                            <td class="text-end"><span
                                                    class="badge bg-warning text-dark bg-opacity-75">{{ $totalKaryawan > 0 ? round(($item['jumlah'] / $totalKaryawan) * 100, 1) : 0 }}%</span>
                                            </td>
                                            <td class="text-end">
                                    ...
                                    <tbody>
                                        @forelse($perKontrak as $item)
                                            <tr class="drill-trigger siska-row-clickable" data-type="kontrak"
                                                ...
                                                <td><span
                                                        class="badge siska-ktr-badge me-1">{{ $item['kode'] }}</span><small>{{ $item['label'] }}</small>
                                                </td>
                                                <td class="text-end"><span
                                                        class="siska-num-badge bg-danger">{{ number_format($item['jumlah']) }}</span>
                                                </td>
                                                <td class="text-end"><span
                                                        class="badge bg-danger bg-opacity-75">{{ $totalKaryawan > 0 ? round(($item['jumlah'] / $totalKaryawan) * 100, 1) : 0 }}%</span>
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
            ...
            <div class="modal-content">
                <div class="modal-header siska-modal-header-purple">
                    <div class="d-flex flex-column">
                        ...
                        <small style="color:rgba(255,255,255,.7);font-size:.75rem;margin-top:2px;">Daftar jabatan — klik
                            angka atau "Lihat" untuk detail karyawan</small>
                    </div>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-0">
...
@push('styles')
    <style>
        ...
        .siska-section-card>.card-body,
        .siska-section-card>.card-body>* {
            border: none !important;
            border-radius: 0 !important;
        }

        .siska-section-card .table-responsive {
            overflow-x: auto;
        }

        .siska-section-card .table {
            margin-bottom: 0;
        }

        /* Flush tabel ke tepi card tanpa padding body */
        ...
        .siska-hdr-cross-pt {
            background: linear-gradient(90deg, #1a6fcf, #4c6ef5);
        }
        ...
        /* Badge kontrak di modal — lebih kecil & tidak melebar */
        #drillTableBody .badge-kontrak {
            display: inline-block;
            font-size: .7rem;
            padding: 2px 7px;
            border-radius: 6px;
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            /* ---- Chart helpers ---- */
            const PAL = ['#1a6fcf', '#0d9488', '#4c6ef5', '#0891b2', '#d97706', '#dc2626', '#7c3aed', '#059669',
            ...
            const TTP = {
                ...
                callbacks: {
                    label: c =>
                        ` ${c.label}: ${Number(c.parsed||c.parsed?.y||0).toLocaleString('id-ID')} karyawan`
                }
            };
            ...
            function mkBar(id, labels, data, horiz, color, onClickFn) {
                ...
                const ch = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        onClick: (e, els) => {
                            if (!els.length || !onClickFn) return;
            ...
            /* ---- Build Charts ---- */
            const ptL = @json($perPT->pluck('singkatan')),
                ptD = @json($perPT->pluck('jumlah')),
                ptI = @json($perPT->pluck('id')),
                ptN = @json($perPT->pluck('label'));
            mkBar('chartPerPT', ptL, ptD, true, null, i => openDrill('pt', ptI[i], ptN[i]));

            const buL = @json($perBidangUsaha->pluck('label')),
                buD = @json($perBidangUsaha->pluck('jumlah'));
            const chartBU = new Chart(document.getElementById('chartBidangUsaha'), {
                ...
                options: {
                    ...
                    onClick: (e, els) => {
                        ...
                        openDrill('bidang', buL[els[0].index], buL[els[0].index]);
                    }
                }
            });

            const pcL = @json($perPusatCabang->pluck('label')),
                pcD = @json($perPusatCabang->pluck('jumlah'));
            new Chart(document.getElementById('chartPusatCabang'), {
                ...
                options: {
                    ...
                    onClick: (e, els) => {
                        if (!els.length) return;
                        openDrill('wilker', pcL[els[0].index], pcL[els[0].index]);
                    }
                }
            });

            const ukL = @json($perUnitKerja->pluck('label')),
                ukD = @json($perUnitKerja->pluck('jumlah')),
                ukI = @json($perUnitKerja->pluck('id'));
            mkBar('chartUnitKerja', ukL, ukD, false, '#0891b2', i => openDrill('unit_kerja', ukI[i], ukL[i]));

            const depL = @json($perDepartemen->pluck('singkatan')),
                depD = @json($perDepartemen->pluck('jumlah')),
                depN = @json($perDepartemen->pluck('label'));
            mkBar('chartDepartemen', depL, depD, true, null, i => openDrill('departemen', depN[i], 'Departemen: ' +
                depN[i]));

            const ktrL = @json($perKontrak->pluck('kode')),
                ktrD = @json($perKontrak->pluck('jumlah')),
                ktrI = @json($perKontrak->pluck('id')),
                ktrN = @json($perKontrak->pluck('label'));
            new Chart(document.getElementById('chartKontrak'), {
                ...
                options: {
                    ...
                    onClick: (e, els) => {
                        if (!els.length) return;
                        const i = els[0].index;
                        openDrill('kontrak', ktrI[i], 'Kontrak: ' + ktrN[i]);
                    }
                }
            });
            ...
            function renderDrillRows(data) {
                const stsClass = s => {
                    if (!s) return 'sts-calon';
                    if (s === 'AKTIF') return 'sts-aktif';
                    if (s === 'NON-AKTIF') return 'sts-nonaktif';
                    return 'sts-calon';
                };
                const genderIcon = s => s === 'LAKI-LAKI' ?
            ...
            <tr>
                ...
                <td style="width:72px">
                    <code style="font-size:.75rem;color:#64748b;background:#f1f5f9;padding:1px 5px;border-radius:4px;">${k.nrk}</code>
                </td>
                <td style="font-weight:600;font-size:.84rem;">${k.nama}</td>
                <td style="width:40px">${genderIcon(k.sex)}</td>
                <td style="width:80px">
                    <span style="font-size:.75rem;background:#eff6ff;color:#1d4ed8;padding:2px 6px;border-radius:5px;font-weight:600;">${k.perusahaan}</span>
                </td>
                ...
                <td style="width:110px">
                    <span style="font-size:.7rem;background:#e0e7ff;color:#3730a3;padding:2px 6px;border-radius:5px;font-weight:600;display:inline-block;max-width:108px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${k.kontrak}">${k.kontrak}</span>
                </td>
                <td style="width:76px"><span class="${stsClass(k.sts_kry)}">${k.sts_kry}</span></td>
