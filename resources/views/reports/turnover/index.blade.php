...
<div class="container-fluid to-page">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('reports.turnover.index') }}" id="toFilterForm">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="siska-topmgmt-toggle">
                            ...
                            <input type="checkbox" name="filter_include_top_mgmt"
                                   id="toToggleTopMgmt" value="1" class="siska-toggle-input"
                    ...
                    {{-- Divider --}}
                    <div class="col-12"><hr style="border-color:#e2e8f0;margin:0;"></div>
                        ...
                        <div class="d-flex align-items-end gap-3">
                            ...
                            <div class="flex-shrink-0" style="padding-bottom:1.25rem;">
                                @foreach ([date('Y'), date('Y') - 1, date('Y') - 2] as $yr)
                                    <a href="{{ route('reports.turnover.index', array_merge($currentFilters, [
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-3">
                        ...
                        @if ($currentFilters['perusahaan'])
                            ...
                            <span class="to-chip to-chip-blue">
                                ...
                                <a href="{{ route('reports.turnover.index', array_merge($currentFilters, ['filter_perusahaan'=>''])) }}" class="ms-1 text-inherit">×</a>
                            </span>
                        @endif
                        @if ($currentFilters['wilker'])
                            <span class="to-chip to-chip-indigo">
                                <i class="fas fa-map me-1"></i>{{ $currentFilters['wilker'] }}
                                <a href="{{ route('reports.turnover.index', array_merge($currentFilters, ['filter_wilker'=>'','filter_area'=>''])) }}" class="ms-1 text-inherit">×</a>
                            </span>
                        @endif
                        @if ($currentFilters['area'])
                            ...
                            <span class="to-chip to-chip-cyan">
                                ...
                                <a href="{{ route('reports.turnover.index', array_merge($currentFilters, ['filter_area'=>''])) }}" class="ms-1 text-inherit">×</a>
                            </span>
                        @endif
                        @if ($currentFilters['departemen'])
                            <span class="to-chip to-chip-orange">
                                <i class="fas fa-layer-group me-1"></i>{{ $currentFilters['departemen'] }}
                                <a href="{{ route('reports.turnover.index', array_merge($currentFilters, ['filter_departemen'=>''])) }}" class="ms-1 text-inherit">×</a>
                            </span>
                        @endif
                        @if ($currentFilters['kontrak'])
                            ...
                            <span class="to-chip to-chip-red">
                                ...
                                <a href="{{ route('reports.turnover.index', array_merge($currentFilters, ['filter_kontrak'=>''])) }}" class="ms-1 text-inherit">×</a>
                            </span>
                        @endif
                        @if ($currentFilters['include_top_mgmt'] === '1')
                            <span class="to-chip to-chip-orange">
                                <i class="fas fa-crown me-1"></i>Termasuk Top Manajement
                                <a href="{{ route('reports.turnover.index', array_merge($currentFilters, ['filter_include_top_mgmt'=>'0'])) }}" class="ms-1 text-inherit">×</a>
                            </span>
                        @endif
                    </div>
                @endif
    ...
    <div class="row g-3 mb-4">
        @php
            $statCards = [
                ['icon'=>'fa-user-minus',   'num'=>$totalResign,              'lbl'=>'Total Resign/PHK',    'cls'=>'to-stat-resign',  'type'=>'all',    'val'=>''],
                ['icon'=>'fa-percentage',   'num'=>$turnoverRate . '%',       'lbl'=>'Turnover Rate',        'cls'=>'to-stat-rate',    'type'=>'',       'val'=>''],
                ['icon'=>'fa-user-plus',    'num'=>$totalMasuk,               'lbl'=>'Karyawan Masuk',       'cls'=>'to-stat-masuk',   'type'=>'',       'val'=>''],
                ['icon'=>'fa-users',        'num'=>round($rataAktif),         'lbl'=>'Rata-rata Aktif',      'cls'=>'to-stat-aktif',   'type'=>'',       'val'=>''],
                ['icon'=>'fa-sign-in-alt',  'num'=>$aktifAwal,                'lbl'=>'Aktif Awal Periode',   'cls'=>'to-stat-awal',    'type'=>'',       'val'=>''],
                ['icon'=>'fa-sign-out-alt', 'num'=>$aktifAkhir,               'lbl'=>'Aktif Akhir Periode',  'cls'=>'to-stat-akhir',   'type'=>'',       'val'=>''],
            ];
        @endphp
                            ...
                            <tbody>
                                @forelse ($resignPerDep as $item)
                                    <tr class="drill-trigger to-row-click" style="cursor:pointer;"
                                        ...
                                        <td class="text-end">
                                            <span class="to-num-badge bg-warning text-dark">{{ number_format($item['jumlah']) }}</span>
                                        </td>
                                        <td class="text-end">
                        ...
                        <table class="table table-sm table-hover mb-0">
                            ...
                            <tbody>
                                @forelse ($resignPerPT as $item)
                                    <tr class="drill-trigger to-row-click"
                                        data-type="pt" data-value="{{ $item['id'] }}"
                                        ...
                                        <td class="text-end">
                                            <span class="to-num-badge bg-primary">{{ number_format($item['jumlah']) }}</span>
                                        </td>
                                        <td class="text-end">
                ...
                <div class="card-body">
                    ...
                    <div class="to-table-mini mt-3">
                        <table class="table table-sm mb-0">
                            <thead class="table-light">
                                <tr><th>Rentang</th><th class="text-end">Orang</th><th class="text-end">%</th></tr>
                            </thead>
                            <tbody>
                                @forelse ($masaKerjaBucket as $item)
                                    <tr>
                                        <td>{{ $item['label'] }}</td>
                                        <td class="text-end">
                                            <span class="to-num-badge bg-secondary">{{ number_format($item['jumlah']) }}</span>
                                        </td>
                                        <td class="text-end">
            ...
            <div class="card to-section-card h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="to-table-mini mt-3">
                        <table class="table table-sm table-hover mb-0">
                            <thead class="table-light">
                                <tr><th>Wilayah</th><th class="text-end">Resign</th></tr>
                            </thead>
                            <tbody>
                                @forelse ($resignPerWilayah as $item)
                                    <tr class="drill-trigger to-row-click"
                                        data-type="wilayah" data-value="{{ $item['label'] }}"
                                        data-label="Wilayah: {{ $item['label'] }}">
                                        <td class="fw-semibold">{{ $item['label'] }}</td>
                                        <td class="text-end">
                                            <span class="to-num-badge" style="background:#4c6ef5;">{{ number_format($item['jumlah']) }}</span>
                                        </td>
                                    </tr>
                                @empty
            ...
            <div class="card to-section-card h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="to-table-mini mt-3">
                        <table class="table table-sm table-hover mb-0">
                            <thead class="table-light">
                                <tr><th>Kontrak</th><th class="text-end">Resign</th><th class="text-end">%</th></tr>
                            </thead>
                            <tbody>
                                @forelse ($resignPerKontrak as $item)
                                    <tr class="drill-trigger to-row-click"
                                        data-type="kontrak" data-value="{{ $item['id'] }}"
                                        ...
                                        <td class="text-end">
                                            <span class="to-num-badge bg-danger">{{ number_format($item['jumlah']) }}</span>
                                        </td>
                                        <td class="text-end">
        ...
        <div class="col-lg-4 d-flex flex-column gap-4">
            ...
            <div class="card to-section-card">
                ...
                <div class="card-body d-flex align-items-center gap-3">
                    <canvas id="chartGender" height="120" style="max-width:120px;"></canvas>
                    <div class="flex-grow-1">
                        @foreach ($resignPerGender as $item)
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="small">
                                    {!! $item['label'] === 'LAKI-LAKI'
                                        ? '<span style="color:#3b82f6;">♂</span> Laki-laki'
                                        : '<span style="color:#a855f7;">♀</span> Perempuan' !!}
                                </span>
                                <span class="fw-bold">{{ number_format($item['jumlah']) }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="card to-section-card flex-grow-1">
                <div class="card-header to-section-header"
                    ...
                    <i class="fas fa-comment-alt me-2" style="opacity:.85;"></i>Alasan PHK / Resign
                </div>
                <div class="card-body p-0">
                    <div class="to-table-mini">
                        <table class="table table-sm table-hover mb-0">
                            <thead class="table-light">
                                <tr><th>#</th><th>Alasan</th><th class="text-end">Orang</th></tr>
                            </thead>
                            <tbody>
                                @forelse ($resignPerAlasan as $i => $item)
                                    <tr>
                                        <td class="text-muted">{{ $i + 1 }}</td>
                                        <td style="font-size:.8rem;">{{ $item['label'] }}</td>
                                        <td class="text-end">
                                            <span class="to-num-badge" style="background:#0d9488;">{{ number_format($item['jumlah']) }}</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="3" class="text-center text-muted">Tidak ada data alasan</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="modal-content">
            <div class="modal-header" style="background:#dc2626;color:#fff;border-bottom:none;padding:.9rem 1.25rem;">
                <div class="d-flex flex-column">
                    ...
                    <small id="toModalSub" style="color:rgba(255,255,255,.7);font-size:.75rem;margin-top:2px;"></small>
                </div>
                ...
                <div id="toContent" style="display:none;">
                    <div class="px-3 pt-2 pb-2 d-flex justify-content-between align-items-center border-bottom"
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <input type="text" id="toSearch" class="form-control form-control-sm border-0 bg-transparent p-0"
                                   placeholder="Cari nama, NRK, departemen, alasan..."
                                   style="width:280px;box-shadow:none;font-size:.83rem;">
                        </div>
                        <span id="toCount" class="badge" style="background:#e2e8f0;color:#475569;font-weight:600;font-size:.72rem;"></span>
                    </div>
...
.to-title { font-size:1.3rem;font-weight:700;color:#1e293b;letter-spacing:-.3px; }

.to-badge-rate   { background:linear-gradient(135deg,#b91c1c,#dc2626);color:#fff;border-radius:8px; }
.to-badge-resign { background:#1e293b;color:#fff;border-radius:8px; }

.to-filter-card  { border:none;border-left:4px solid #dc2626;border-radius:8px; }
.to-label        { font-size:.73rem;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px; }
.to-hint         { font-size:.65rem;color:#94a3b8;line-height:1.3; }
...
/* Chips */
.to-chip { display:inline-flex;align-items:center;padding:3px 10px;border-radius:12px;font-size:.75rem;font-weight:600; }
.to-chip a { text-decoration:none;opacity:.7; }
.to-chip a:hover { opacity:1; }
.to-chip-blue   { background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe; }
.to-chip-indigo { background:#eef2ff;color:#4338ca;border:1px solid #c7d2fe; }
.to-chip-cyan   { background:#ecfeff;color:#0e7490;border:1px solid #a5f3fc; }
.to-chip-orange { background:#fff7ed;color:#c2410c;border:1px solid #fed7aa; }
.to-chip-red    { background:#fef2f2;color:#991b1b;border:1px solid #fecaca; }
...
.to-stat-card {
    ...
    box-shadow:0 2px 8px rgba(0,0,0,.12);transition:transform .18s,box-shadow .18s;
}
.to-stat-card.drill-trigger { cursor:pointer; }
.to-stat-card.drill-trigger:hover { transform:translateY(-3px);box-shadow:0 8px 22px rgba(0,0,0,.2); }
.to-stat-card::before {
    content:'';position:absolute;top:-18px;right:-18px;
    width:70px;height:70px;border-radius:50%;background:rgba(255,255,255,.1);
}
.to-stat-icon  { font-size:1.25rem;opacity:.75;background:rgba(255,255,255,.15);width:36px;height:36px;border-radius:8px;display:flex;align-items:center;justify-content:center; }
.to-stat-num   { font-size:2rem;font-weight:800;line-height:1;letter-spacing:-1px;margin-top:.35rem; }
.to-stat-lbl   { font-size:.72rem;font-weight:500;opacity:.85;margin-top:.2rem; }
.to-stat-hint  { font-size:.62rem;opacity:0;transition:opacity .2s;margin-top:.15rem; }
.to-stat-card.drill-trigger:hover .to-stat-hint { opacity:.75; }

.to-stat-resign { background:linear-gradient(145deg,#b91c1c,#ef4444); }
.to-stat-rate   { background:linear-gradient(145deg,#7c2d12,#ea580c); }
.to-stat-masuk  { background:linear-gradient(145deg,#047857,#10b981); }
.to-stat-aktif  { background:linear-gradient(145deg,#1e293b,#334155); }
.to-stat-awal   { background:linear-gradient(145deg,#1d4ed8,#3b82f6); }
.to-stat-akhir  { background:linear-gradient(145deg,#5b21b6,#7c3aed); }

/* Section cards */
.to-section-card { border:1px solid #e9eef5!important;border-radius:12px!important;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.06); }
.to-section-header { font-weight:600;font-size:.875rem;padding:.7rem 1.1rem;color:#fff;letter-spacing:.1px; }

.to-table-mini { max-height:200px;overflow-y:auto; }
.to-table-mini::-webkit-scrollbar { width:4px; }
.to-table-mini::-webkit-scrollbar-thumb { background:#cbd5e1;border-radius:4px; }

.to-row-click:hover { background:#fff0f0!important; }

.to-num-badge { display:inline-block;color:#fff;padding:2px 10px;border-radius:12px;font-weight:700;font-size:.82rem;cursor:pointer;transition:transform .15s; }
.to-num-badge:hover { transform:scale(1.1); }

.to-dep-badge { background:#f1f5f9;color:#475569;font-size:.65rem;padding:2px 6px;border-radius:4px; }
.to-ktr-badge { background:#fef2f2;color:#991b1b;font-size:.65rem;padding:2px 6px;border-radius:4px; }
...
#toTable tbody tr td { padding:.5rem .75rem!important;vertical-align:middle;font-size:.82rem; }
#toTable thead th   { font-size:.75rem;padding:.5rem .75rem!important;font-weight:600; }

@media(max-width:768px) { .to-stat-num { font-size:1.4rem; } }
</style>
@endpush
...
document.addEventListener('DOMContentLoaded', function () {
        ...
        filterWilker.addEventListener('change', function () {
            fetch(AREA_URL + (this.value ? '?wilker=' + encodeURIComponent(this.value) : ''))
                ...
                .then(data => {
                    ...
                    data.forEach(ak => {
                        filterArea.innerHTML += `<option value="${ak.id}">${ak.area_krj}${ak.singkatan_wk ? ' (' + ak.singkatan_wk + ')' : ''}</option>`;
                    });
                });
        });
    }
    ...
    // ── Chart palette ──
    const PAL = ['#dc2626','#d97706','#4c6ef5','#0d9488','#7c3aed','#059669','#f59e0b','#0891b2','#ec4899','#6366f1'];
    ...
    const TTP = {
        ...
        callbacks: {
            label: c => {
                const v = typeof c.parsed === 'object' ? (c.parsed.y ?? c.parsed.x ?? c.parsed) : c.parsed;
    ...
    if (depData.length && document.getElementById('chartResignDep')) {
        new Chart(document.getElementById('chartResignDep'), {
            ...
            data: {
                ...
                datasets: [{ label: 'Resign', data: depData.map(r => r.jumlah),
                    backgroundColor: PAL, borderRadius: 5 }]
            },
            options: {
                indexAxis: 'y', responsive: true,
                plugins: { legend: { display: false }, tooltip: TTP },
                scales: { x: { grid: { color: '#f1f5f9' } }, y: { grid: { color: 'transparent' } } },
                onClick: (e, els) => {
                    if (els.length) openModal('departemen', depData[els[0].index].label, 'Resign — ' + depData[els[0].index].label);
                }
            }
        });
    }
    ...
    if (mkData.length && document.getElementById('chartMasaKerja')) {
        new Chart(document.getElementById('chartMasaKerja'), {
            ...
            data: {
                ...
                datasets: [{ data: mkData.map(r => r.jumlah), backgroundColor: PAL,
                    borderWidth: 2, borderColor: '#fff', hoverOffset: 8 }]
            },
            options: { cutout: '55%', responsive: true, plugins: { tooltip: TTP,
    ...
    if (wlData.length && document.getElementById('chartWilayah')) {
        new Chart(document.getElementById('chartWilayah'), {
            ...
            data: {
                ...
                datasets: [{ data: wlData.map(r => r.jumlah),
                    backgroundColor: ['#4c6ef5','#6366f1','#818cf8','#a5b4fc','#c7d2fe'],
                    borderWidth: 2, borderColor: '#fff', hoverOffset: 8 }]
            },
            options: { responsive: true, plugins: { tooltip: TTP,
                legend: { position: 'bottom', labels: { padding: 10, boxWidth: 12 } } },
                onClick: (e, els) => {
                    if (els.length) openModal('wilayah', wlData[els[0].index].label, 'Wilayah: ' + wlData[els[0].index].label);
                }
            }
        });
    }
    ...
    if (ktrData.length && document.getElementById('chartKontrak')) {
        new Chart(document.getElementById('chartKontrak'), {
            ...
            data: {
                ...
                datasets: [{ data: ktrData.map(r => r.jumlah),
    ...
    if (gnData.length && document.getElementById('chartGender')) {
        new Chart(document.getElementById('chartGender'), {
            ...
            data: {
                ...
                datasets: [{ data: gnData.map(r => r.jumlah),
            ...
            options: { cutout: '55%', responsive: true, plugins: { tooltip: TTP,
                ...
                onClick: (e, els) => {
                    if (els.length) openModal('gender', gnData[els[0].index].label, gnData[els[0].index].label);
                }
            }
        });
    }
    ...
    function openModal(type, value, label) {
        ...
        fetch(DETAIL_URL + '?' + params)
            ...
            .then(res => {
                ...
                document.getElementById('toFooterInfo').textContent = 'Total: ' + res.total.toLocaleString('id-ID') + ' karyawan resign';
                document.getElementById('toCount').textContent      = res.total + ' karyawan';
                if (!res.data?.length) { document.getElementById('toEmpty').style.display = 'block'; return; }
                renderRows(res.data);
